...
void C_Grammar_grammaireBDD::_performSourceStringParsing_ (C_Compiler & inCompiler,
  ...
  C_Lexique_lexiqueBDD * scanner_ = NULL ;
  macroMyNew (scanner_, C_Lexique_lexiqueBDD (& inCompiler, inCompiler.ioParametersPtr (), inSourceString.string (), "Error when parsing dynamic string" COMMA_HERE)) ;
  scanner_->mPerformGeneration = inCompiler.mPerformGeneration ;
  const bool ok = scanner_->performTopDownParsing (gProductions, gProductionNames, gProductionIndexes,
                                                   gFirstProductionIndexes, gDecision, gDecisionIndexes, 236) ;
  if (ok && ! scanner_->mParseOnlyFlag) {
