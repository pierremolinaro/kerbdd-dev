...
void routine_verifierNombreArgumentsBool (C_Lexique & lexique_var_,
  ...
  GGS_typeFormalArgumentsList::element_type * operand_8238 = var_cas_t1.firstObject () ;
  GGS_typeActualArgumentsList::element_type * operand_8327 = var_cas_t2.firstObject () ;
  while ((operand_8238 != NULL)
      && (operand_8327 != NULL)) {
    macroValidPointer (operand_8238) ;
    macroValidPointer (operand_8327) ;
    if (((operand_8238->mVariableBitSize.reader_value ()) == (operand_8327->mVariableBitSize.reader_value ())).isBuiltAndTrue ()) {
      if (((operand_8238->mLowBound.reader_value ()) == (operand_8327->mLowBound.reader_value ())).isBuiltAndTrue ()) {
        if (((operand_8238->mHighBound.reader_value ()) == (operand_8327->mHighBound.reader_value ())).isBuiltAndTrue ()) {
        }else{
            operand_8327->mErrorLocation.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "the actual high bound is different the formal one")) ;
        }
      }else{
          operand_8327->mErrorLocation.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "the actual low bound is different the formal one")) ;
      }
    }else{
        operand_8327->mErrorLocation.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "the actual dimension is different from the formal one")) ;
    }
    operand_8238 = operand_8238->nextObject () ;
    operand_8327 = operand_8327->nextObject () ;
  }
}
