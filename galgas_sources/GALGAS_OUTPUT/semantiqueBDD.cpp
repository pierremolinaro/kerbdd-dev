...
void GGS_typeDomainMap::methode_searchKeyGetIndex (C_Lexique & inLexique,
                                const GGS_lstring & inKey,
                                GGS_luint & outIndex,
                                GGS_typeDomain   & outParameter0) {
  const char * kErrorMessage [] = {"the '%' domain is not declared",
...
void GGS_typeDomainMap::methode_insertKeyGetIndex (C_Lexique & lexique_var_,
                                const GGS_lstring & inKey,
                                GGS_luint & outIndex,
                                const GGS_typeDomain & inParameter0) {
  const char * kErrorMessage [] = {"the '%' domain has been already declared",
...
//---------------------------------------------------------------------------*
  ...
  GGS_typeTableVariablesBool::element_type * info = searchKey (inLexique, inKey, inKey, errorMessage_0) ;
  if (info == NULL) {
    outParameter0.drop_operation () ;
  }else{
...
void GGS_typeTableVariablesBool::methode_insertKey (C_Lexique & lexique_var_,
  ...
  insertKey (lexique_var_, info, inKey, inKey, kErrorMessage_0) ;
}
...
void GGS_typeTableVariablesBool::methode_insertKeyGetIndex (C_Lexique & lexique_var_,
  ...
  e_typeTableVariablesBool info ;
  info.mVariableDescriptor = inParameter0 ;
  const sint32 index = insertKey (lexique_var_, info, inKey, inKey, kErrorMessage_0) ;
...
void routine_verifierNombreArgumentsBool (C_Lexique & lexique_var_,
  ...
  GGS_typeFormalArgumentsList::element_type * operand_7906 = var_cas_t1.firstObject () ;
  GGS_typeActualArgumentsList::element_type * operand_7995 = var_cas_t2.firstObject () ;
  while ((operand_7906 != NULL)
      && (operand_7995 != NULL)) {
    macroValidPointer (operand_7906) ;
    macroValidPointer (operand_7995) ;
    if (((operand_7906->mVariableBitSize.reader_value ()) == (operand_7995->mVariableBitSize.reader_value ())).isBuiltAndTrue ()) {
      if (((operand_7906->mLowBound.reader_value ()) == (operand_7995->mLowBound.reader_value ())).isBuiltAndTrue ()) {
        if (((operand_7906->mHighBound.reader_value ()) == (operand_7995->mHighBound.reader_value ())).isBuiltAndTrue ()) {
        }else{
            operand_7995->mErrorLocation.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "the actual high bound is different the formal one")) ;
        }
      }else{
          operand_7995->mErrorLocation.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "the actual low bound is different the formal one")) ;
      }
    }else{
        operand_7995->mErrorLocation.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "the actual dimension is different from the formal one")) ;
    }
    operand_7906 = operand_7906->nextObject () ;
    operand_7995 = operand_7995->nextObject () ;
  }
}
