...
void routine_verifierNombreArgumentsBool (C_Lexique & lexique_var_,
  ...
  GGS_typeFormalArgumentsList::element_type * operand_8187 = var_cas_t1.firstObject () ;
  GGS_typeActualArgumentsList::element_type * operand_8276 = var_cas_t2.firstObject () ;
  while ((operand_8187 != NULL)
      && (operand_8276 != NULL)) {
    macroValidPointer (operand_8187) ;
    macroValidPointer (operand_8276) ;
    if (((operand_8187->mVariableBitSize.reader_value ()) == (operand_8276->mVariableBitSize.reader_value ())).isBuiltAndTrue ()) {
      if (((operand_8187->mLowBound.reader_value ()) == (operand_8276->mLowBound.reader_value ())).isBuiltAndTrue ()) {
        if (((operand_8187->mHighBound.reader_value ()) == (operand_8276->mHighBound.reader_value ())).isBuiltAndTrue ()) {
        }else{
            operand_8276->mErrorLocation.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "the actual high bound is different the formal one")) ;
        }
      }else{
          operand_8276->mErrorLocation.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "the actual low bound is different the formal one")) ;
      }
    }else{
        operand_8276->mErrorLocation.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "the actual dimension is different from the formal one")) ;
    }
    operand_8187 = operand_8187->nextObject () ;
    operand_8276 = operand_8276->nextObject () ;
  }
}
