...
aNumeroVariable.reader_description () ;
mLowBound.reader_description () ;
mHighBound.reader_description () ;
}
...
aNumeroFormule.reader_description () ;
aListeArgsBool.reader_description () ;
aFormuleTraduite.reader_description () ;
aEstFormuleCourante.reader_description () ;
}
...
aOperande1.reader_description () ;
aOperande2.reader_description () ;
}
...
aComparaison.reader_description () ;
aOperande1.reader_description () ;
aOperande2.reader_description () ;
}
...
aListeArgsBooleens.reader_description () ;
aOperande.reader_description () ;
aContraintes.reader_description () ;
}
...
aIndiceBDD.reader_description () ;
aDimension.reader_description () ;
mLowBound.reader_description () ;
mHighBound.reader_description () ;
aComparaison.reader_description () ;
aValeur.reader_description () ;
}
...
aIndiceBDDgauche.reader_description () ;
aDimension.reader_description () ;
mLeftLowBound.reader_description () ;
mLeftHighBound.reader_description () ;
aComparaison.reader_description () ;
aIndiceBDDdroite.reader_description () ;
...
aNumeroFormule.reader_description () ;
aFormule.reader_description () ;
aNomFormule.reader_description () ;
aTableVariablesBool.reader_description () ;
aAfficher.reader_description () ;
}
...
aDimensionCache.reader_description () ;
}
...
aDimensionTable.reader_description () ;
}
...
appendForListDescription (C_String & ioString) const {
  ioString << "[" ;
  ioString << aCalcul.reader_description () ;
  ioString << "]" ;
}
...
void routine_verifierNombreArgumentsBool (C_Lexique & _inLexique,
  ...
  if (((var_cas_t1.reader_length ()) < (var_cas_t2.reader_length ())).isBuiltAndTrue ()) {
      var_cas_positionFinListe.reader_location ().signalGGSSemanticError (_inLexique, GGS_string (true, "too much actual arguments") SOURCE_FILE_AT_LINE (260)) ;
  }else if (((var_cas_t1.reader_length ()) > (var_cas_t2.reader_length ())).isBuiltAndTrue ()) {
      var_cas_positionFinListe.reader_location ().signalGGSSemanticError (_inLexique, GGS_string (true, "one or more actual arguments missing") SOURCE_FILE_AT_LINE (262)) ;
  }
  GGS_typeFormalArgumentsList::element_type * operand_7824 = var_cas_t1.firstObject () ;
  GGS_typeActualArgumentsList::element_type * operand_7913 = var_cas_t2.firstObject () ;
  while ((operand_7824 != NULL)
      && (operand_7913 != NULL)) {
    macroValidPointer (operand_7824) ;
    macroValidPointer (operand_7913) ;
    if (((operand_7824->mVariableBitSize.reader_value ()) == (operand_7913->mVariableBitSize.reader_value ())).isBuiltAndTrue ()) {
      if (((operand_7824->mLowBound.reader_value ()) == (operand_7913->mLowBound.reader_value ())).isBuiltAndTrue ()) {
        if (((operand_7824->mHighBound.reader_value ()) == (operand_7913->mHighBound.reader_value ())).isBuiltAndTrue ()) {
        }else{
            operand_7913->mErrorLocation.reader_location ().signalGGSSemanticError (_inLexique, GGS_string (true, "the actual high bound is different the formal one") SOURCE_FILE_AT_LINE (270)) ;
        }
      }else{
          operand_7913->mErrorLocation.reader_location ().signalGGSSemanticError (_inLexique, GGS_string (true, "the actual low bound is different the formal one") SOURCE_FILE_AT_LINE (273)) ;
      }
    }else{
        operand_7913->mErrorLocation.reader_location ().signalGGSSemanticError (_inLexique, GGS_string (true, "the actual dimension is different from the formal one") SOURCE_FILE_AT_LINE (276)) ;
    }
    operand_7824 = operand_7824->nextObject () ;
    operand_7913 = operand_7913->nextObject () ;
  }
}
...
void routine_buildFormalArgsList (C_Lexique & _inLexique,
  ...
  if (var_cas_inVariableDescriptor.isBuilt ()) {
    if (dynamic_cast <cPtr_typeDirectVariable *> (var_cas_inVariableDescriptor.getPtr ()) != NULL) {
      cPtr_typeDirectVariable * operand_8802 = dynamic_cast <cPtr_typeDirectVariable *> (var_cas_inVariableDescriptor.getPtr ()) ;
      macroValidPointer (operand_8802) ; 
      var_cas_ioFormalArgsList.addAssign_operation (operand_8802->mIndex, operand_8802->mBitSize, operand_8802->mLowBound, operand_8802->mHighBound) ;
    }else if (dynamic_cast <cPtr_typeRecordVariable *> (var_cas_inVariableDescriptor.getPtr ()) != NULL) {
      cPtr_typeRecordVariable * operand_8988 = dynamic_cast <cPtr_typeRecordVariable *> (var_cas_inVariableDescriptor.getPtr ()) ;
      macroValidPointer (operand_8988) ; 
      GGS_typeTableVariablesBool::element_type * operand_9085 = operand_8988->mMap.firstObject () ;
      while ((operand_9085 != NULL)) {
        macroValidPointer (operand_9085) ;
        ::routine_buildFormalArgsList (_inLexique,  operand_9085->mInfo.mVariableDescriptor,  var_cas_ioFormalArgsList,  var_cas_inErrorLocation SOURCE_FILE_AT_LINE (293)) ;
        operand_9085 = operand_9085->nextObject () ;
      }
    }else{ // Else part
        var_cas_inErrorLocation.reader_location ().signalGGSSemanticError (_inLexique, ((GGS_string (true, "a variable was expected here ; I found ")) + (var_cas_inVariableDescriptor.reader_variableDescriptorErrorMessage ())) SOURCE_FILE_AT_LINE (296)) ;
    }
  }
}
...
void routine_buildActualArgsList (C_Lexique & _inLexique,
                                ...
                                const GGS_location  & var_cas_inErrorLocation COMMA_UNUSED_LOCATION_ARGS) {
  GGS_typeTableVariablesBool::element_type * operand_9660 = var_cas_inDomainMap.firstObject () ;
  while ((operand_9660 != NULL)) {
    macroValidPointer (operand_9660) ;
    if (operand_9660->mInfo.mVariableDescriptor.isBuilt ()) {
      if (dynamic_cast <cPtr_typeDirectVariable *> (operand_9660->mInfo.mVariableDescriptor.getPtr ()) != NULL) {
        cPtr_typeDirectVariable * operand_9769 = dynamic_cast <cPtr_typeDirectVariable *> (operand_9660->mInfo.mVariableDescriptor.getPtr ()) ;
        macroValidPointer (operand_9769) ; 
        var_cas_ioActualArgsList.addAssign_operation (operand_9769->mIndex, operand_9769->mBitSize, operand_9769->mLowBound, operand_9769->mHighBound, var_cas_inErrorLocation) ;
      }else if (dynamic_cast <cPtr_typeRecordVariable *> (operand_9660->mInfo.mVariableDescriptor.getPtr ()) != NULL) {
        cPtr_typeRecordVariable * operand_9976 = dynamic_cast <cPtr_typeRecordVariable *> (operand_9660->mInfo.mVariableDescriptor.getPtr ()) ;
        macroValidPointer (operand_9976) ; 
        ::routine_buildActualArgsList (_inLexique,  operand_9976->mMap,  var_cas_ioActualArgsList,  var_cas_inErrorLocation SOURCE_FILE_AT_LINE (312)) ;
      }else{ // Else part
          var_cas_inErrorLocation.reader_location ().signalGGSSemanticError (_inLexique, ((GGS_string (true, "a variable was expected here ; I found ")) + (operand_9660->mInfo.mVariableDescriptor.reader_variableDescriptorErrorMessage ())) SOURCE_FILE_AT_LINE (315)) ;
      }
    }
    operand_9660 = operand_9660->nextObject () ;
  }
}
