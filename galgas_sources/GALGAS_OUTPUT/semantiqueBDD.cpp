...
appendForMapDescription (C_String & ioString) const {
  ioString << "["
           << mKey.reader_description () ;
  ioString << "->" ;
  ioString << mInfo.mVariableDescriptor.reader_description () ;
  ioString << "]" ;
}
...
void routine_verifierNombreArgumentsBool (C_Lexique & _inLexique,
  ...
  if (((var_cas_t1.reader_length ()) < (var_cas_t2.reader_length ())).isBuiltAndTrue ()) {
      var_cas_positionFinListe.reader_location ().signalGGSSemanticError (_inLexique, GGS_string (true, "too much actual arguments") SOURCE_FILE_AT_LINE (259)) ;
  }else if (((var_cas_t1.reader_length ()) > (var_cas_t2.reader_length ())).isBuiltAndTrue ()) {
      var_cas_positionFinListe.reader_location ().signalGGSSemanticError (_inLexique, GGS_string (true, "one or more actual arguments missing") SOURCE_FILE_AT_LINE (261)) ;
  }
  ...
  while ((operand_7785 != NULL)
    ...
    if (((operand_7785->mVariableBitSize.reader_value ()) == (operand_7874->mVariableBitSize.reader_value ())).isBuiltAndTrue ()) {
      if (((operand_7785->mLowBound.reader_value ()) == (operand_7874->mLowBound.reader_value ())).isBuiltAndTrue ()) {
        if (((operand_7785->mHighBound.reader_value ()) == (operand_7874->mHighBound.reader_value ())).isBuiltAndTrue ()) {
        }else{
            operand_7874->mErrorLocation.reader_location ().signalGGSSemanticError (_inLexique, GGS_string (true, "the actual high bound is different the formal one") SOURCE_FILE_AT_LINE (269)) ;
        }
      }else{
          operand_7874->mErrorLocation.reader_location ().signalGGSSemanticError (_inLexique, GGS_string (true, "the actual low bound is different the formal one") SOURCE_FILE_AT_LINE (272)) ;
      }
    }else{
        operand_7874->mErrorLocation.reader_location ().signalGGSSemanticError (_inLexique, GGS_string (true, "the actual dimension is different from the formal one") SOURCE_FILE_AT_LINE (275)) ;
    }
  ...
  if (var_cas_inVariableDescriptor.isBuilt ()) {
    if (dynamic_cast <cPtr_typeDirectVariable *> (var_cas_inVariableDescriptor.getPtr ()) != NULL) {
      ...
      while ((operand_9046 != NULL)) {
        ...
        var_cas_inErrorLocation.reader_location ().signalGGSSemanticError (_inLexique, ((GGS_string (true, "a variable was expected here ; I found ")) + (var_cas_inVariableDescriptor.reader_variableDescriptorErrorMessage ())) SOURCE_FILE_AT_LINE (295)) ;
    }
  }
}
    ...
    if (operand_9621->mInfo.mVariableDescriptor.isBuilt ()) {
      if (dynamic_cast <cPtr_typeDirectVariable *> (operand_9621->mInfo.mVariableDescriptor.getPtr ()) != NULL) {
        ...
        ::routine_buildActualArgsList (_inLexique,  operand_9937->mMap,  var_cas_ioActualArgsList,  var_cas_inErrorLocation SOURCE_FILE_AT_LINE (311)) ;
          ...
          var_cas_inErrorLocation.reader_location ().signalGGSSemanticError (_inLexique, ((GGS_string (true, "a variable was expected here ; I found ")) + (operand_9621->mInfo.mVariableDescriptor.reader_variableDescriptorErrorMessage ())) SOURCE_FILE_AT_LINE (314)) ;
      }
    }
