...
void GGS_typeTableVariablesBool::methode_insertKey (C_Lexique & lexique_var_,
  ...
  insertKey (lexique_var_, info, inKey, inKey, "the '%' variable has been already declared" COMMA_THERE) ;
}
...
void GGS_typeTableVariablesBool::methode_insertKeyGetIndex (C_Lexique & lexique_var_,
  ...
  e_typeTableVariablesBool info ;
  info.mVariableDescriptor = inParameter0 ;
  const sint32 index = insertKey (lexique_var_, info, inKey, inKey, "the '%' variable has been already declared" COMMA_THERE) ;
  outIndex = GGS_luint (GGS_uint (index >= 0, (uint32) index), inKey) ;
}
...
void GGS_typeTableFormules::
methode_insertKeyGetIndex (C_Lexique & lexique_var_,
                                const GGS_lstring & inKey,
                                GGS_luint & outIndex,
                                const GGS_typeFormalArgumentsList & inParameter0,
                                const GGS_bdd& inParameter1 COMMA_LOCATION_ARGS) {
...
void routine_verifierNombreArgumentsBool (C_Lexique & lexique_var_,
  ...
  if (((var_cas_t1.reader_length ()) < (var_cas_t2.reader_length ())).isBuiltAndTrue ()) {
      var_cas_positionFinListe.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "too much actual arguments") SOURCE_FILE_AT_LINE (260)) ;
  }else if (((var_cas_t1.reader_length ()) > (var_cas_t2.reader_length ())).isBuiltAndTrue ()) {
      var_cas_positionFinListe.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "one or more actual arguments missing") SOURCE_FILE_AT_LINE (262)) ;
  }
    ...
    if (((operand_7824->mVariableBitSize.reader_value ()) == (operand_7913->mVariableBitSize.reader_value ())).isBuiltAndTrue ()) {
      if (((operand_7824->mLowBound.reader_value ()) == (operand_7913->mLowBound.reader_value ())).isBuiltAndTrue ()) {
        if (((operand_7824->mHighBound.reader_value ()) == (operand_7913->mHighBound.reader_value ())).isBuiltAndTrue ()) {
            ...
            operand_7913->mErrorLocation.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "the actual high bound is different the formal one") SOURCE_FILE_AT_LINE (270)) ;
        }
      }else{
          operand_7913->mErrorLocation.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "the actual low bound is different the formal one") SOURCE_FILE_AT_LINE (273)) ;
      }
    }else{
        operand_7913->mErrorLocation.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "the actual dimension is different from the formal one") SOURCE_FILE_AT_LINE (276)) ;
    }
