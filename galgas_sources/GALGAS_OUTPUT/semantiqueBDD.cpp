...
void routine_verifierNombreArgumentsBool (C_Lexique & lexique_var_,
  ...
  GGS_typeFormalArgumentsList::element_type * operand_7888 = var_cas_t1.firstObject () ;
  GGS_typeActualArgumentsList::element_type * operand_7977 = var_cas_t2.firstObject () ;
  while ((operand_7888 != NULL)
      && (operand_7977 != NULL)) {
    macroValidPointer (operand_7888) ;
    macroValidPointer (operand_7977) ;
    if (((operand_7888->mVariableBitSize.reader_value ()) == (operand_7977->mVariableBitSize.reader_value ())).isBuiltAndTrue ()) {
      if (((operand_7888->mLowBound.reader_value ()) == (operand_7977->mLowBound.reader_value ())).isBuiltAndTrue ()) {
        if (((operand_7888->mHighBound.reader_value ()) == (operand_7977->mHighBound.reader_value ())).isBuiltAndTrue ()) {
        }else{
            operand_7977->mErrorLocation.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "the actual high bound is different the formal one")) ;
        }
      }else{
          operand_7977->mErrorLocation.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "the actual low bound is different the formal one")) ;
      }
    }else{
        operand_7977->mErrorLocation.reader_location ().signalGGSSemanticError (lexique_var_, GGS_string (true, "the actual dimension is different from the formal one")) ;
    }
    operand_7888 = operand_7888->nextObject () ;
    operand_7977 = operand_7977->nextObject () ;
  }
}
