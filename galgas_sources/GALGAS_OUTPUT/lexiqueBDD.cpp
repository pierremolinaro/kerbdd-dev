  ...
  while ((token.mTokenCode < 0) && (UNICODE_VALUE (mCurrentChar) != '\0')) {
    ...
    try{
      if (testForInputUTF32CharRange (TO_UNICODE ('a'), TO_UNICODE ('z')) ||
        ...
        do {
          ::scanner_action_enterCharacterIntoString (*this, token.att_idf, ::scanner_action_toLower (*this, previousChar ())) ;
        ...
        ::scanner_action_enterDigitIntoUlong (*this, token.att_valeur, previousChar (), gErrorMessage_0, gErrorMessage_1) ;
        do {
        ...
        ::scanner_action_formerCompEgal (*this, token.att_valeur) ;
        token.mTokenCode = lexiqueBDD_1_comp ;
        enterToken (token) ;
      }else if (testForInputUTF32Char (TO_UNICODE ('<'))) {
        if (testForInputUTF32Char (TO_UNICODE ('>'))) {
          ::scanner_action_formerCompNonEgal (*this, token.att_valeur) ;
          token.mTokenCode = lexiqueBDD_1_comp ;
          enterToken (token) ;
        }else if (testForInputUTF32Char (TO_UNICODE ('='))) {
          ::scanner_action_formerCompInfEgal (*this, token.att_valeur) ;
          token.mTokenCode = lexiqueBDD_1_comp ;
          enterToken (token) ;
        }else{
          ::scanner_action_formerCompInf (*this, token.att_valeur) ;
          token.mTokenCode = lexiqueBDD_1_comp ;
          enterToken (token) ;
        }
      }else if (testForInputUTF32Char (TO_UNICODE ('>'))) {
        if (testForInputUTF32Char (TO_UNICODE ('='))) {
          ::scanner_action_formerCompSupEgal (*this, token.att_valeur) ;
          token.mTokenCode = lexiqueBDD_1_comp ;
          enterToken (token) ;
        }else{
          ::scanner_action_formerCompSup (*this, token.att_valeur) ;
          token.mTokenCode = lexiqueBDD_1_comp ;
          enterToken (token) ;
        }
      }else if (testForInputUTF32Char (TO_UNICODE ('$'))) {
