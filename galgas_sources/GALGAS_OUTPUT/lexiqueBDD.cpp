  ...
  if (_p == NULL) {
    ...
    case  lexiqueBDD_1_nombre:
      s << "$"
        ...
        << "$" ;
      s << " " ;
      s.appendUnsigned (_p->att_valeur) ;
      break;
...
parseLexicalToken (void) {
  ...
  while ((_token.mTokenCode < 0) && (UNICODE_VALUE (mCurrentChar) != '\0')) {
    _token.att_idf.setLengthToZero () ;
    _token.att_token.setLengthToZero () ;
    _token.att_valeur = 0 ;
    ...
    try{
      if (testForInputUTF32CharRange (TO_UNICODE ('a'), TO_UNICODE ('z')) ||
        ...
        loop_ = true ;
        _token.mTokenCode = search_into_motsReservesKerBDD (_token.att_idf) ;
        if (_token.mTokenCode == -1) {
          _token.mTokenCode = lexiqueBDD_1_id ;
        }
        _enterToken (_token) ;
        ...
        loop_ = true ;
        if (testForInputUTF32Char (TO_UNICODE ('\"'))) {
          _token.mTokenCode = lexiqueBDD_1_string ;
          _enterToken (_token) ;
        }else{
        ...
        _token.mTokenCode = lexiqueBDD_1_ ; // Empty string code
