...
void routine_exploreRecordDomain (C_Compiler & _inLexique,
  ...
  GGS_typeDomainMap::element_type * operand_8447 = var_cas_inRecordMap.firstObject () ;
  while ((operand_8447 != NULL)) {
    macroValidPointer (operand_8447) ;
    if (operand_8447->mInfo.mDomain._isBuilt ()) {
      cPtr_typeDirectDomain * operand_8519 = dynamic_cast <cPtr_typeDirectDomain *> (operand_8447->mInfo.mDomain.getPtr ()) ;
      if (operand_8519 != NULL) {
        ...
        var_cas_outMap.modifier_insertKey (_inLexique, operand_8447->mKey, var_cas_v COMMA_SOURCE_FILE_AT_LINE (348)) ;
        ...
        cPtr_typeRecordDomain * operand_8852 = dynamic_cast <cPtr_typeRecordDomain *> (operand_8447->mInfo.mDomain.getPtr ()) ;
        if (operand_8852 != NULL) {
          ...
          var_cas_outMap.modifier_insertKey (_inLexique, operand_8447->mKey, var_cas_v COMMA_SOURCE_FILE_AT_LINE (354)) ;
        }else{ // Else part
          var_cas_inErrorLocation.reader_location (_inLexique COMMA_HERE).signalGGSSemanticError (_inLexique, (GGS_string (true, "a aDomain was expected here ; I found "))._operator_concat (operand_8447->mInfo.mDomain.reader_messageDomainType (_inLexique COMMA_SOURCE_FILE_AT_LINE (356))) COMMA_SOURCE_FILE_AT_LINE (357)) ;
        }
      }
    }
    operand_8447 = operand_8447->nextObject () ;
  }
  #ifdef DEBUG_TRACE_ENABLED
    ...
    while (syntaxRepeat) {
      switch (select_repeat_syntaxeBDD_13 (_inLexique)) {
        case 2 : {
          ...
          { cPtr_typeRecordVariable * ptrExtraire_ = dynamic_cast <cPtr_typeRecordVariable *> (var_cas_outVarDescriptor.getPtr ()) ;
            if (ptrExtraire_ == NULL) {
              ...
              if (var_cas_outVarDescriptor.getPtr () != NULL) {
                ...
                var_cas_errorLoc.signalExtractError (_inLexique, message1_, message2_ COMMA_SOURCE_FILE_AT_LINE (951)) ;
              }
            }else{
              macroValidPointer (ptrExtraire_) ;
          ...
          _inLexique._assignFromAttribute_att_token (var_cas_fieldName) ;
          _inLexique.acceptTerminal (ACCEPT_TERMINAL (lexiqueBDD::lexiqueBDD_1_id) COMMA_HERE) ;
          var_cas_outErrorLocation = var_cas_fieldName.reader_location (_inLexique COMMA_SOURCE_FILE_AT_LINE (954)) ;
          if (var_cas_fieldMap._isBuilt ()) {
            var_cas_fieldMap (HERE)->method_searchKey (_inLexique, var_cas_fieldName, var_cas_outVarDescriptor COMMA_SOURCE_FILE_AT_LINE (955)) ;
          }
          } break ;
        default :
          syntaxRepeat = false ;
  ...
  { cPtr_typeDirectVariable * ptrExtraire_ = dynamic_cast <cPtr_typeDirectVariable *> (var_cas_v.getPtr ()) ;
    if (ptrExtraire_ == NULL) {
      ...
      if (var_cas_v.getPtr () != NULL) {
        ...
        message1_ (0 COMMA_HERE) << cPtr_typeDirectVariable::static_string_message_variableDescriptorErrorMessage () ;
        message2_ << var_cas_v (HERE)->message_variableDescriptorErrorMessage () ;
        var_cas_outErrorLocation.signalExtractError (_inLexique, message1_, message2_ COMMA_SOURCE_FILE_AT_LINE (972)) ;
      }
    }else{
      macroValidPointer (ptrExtraire_) ;
